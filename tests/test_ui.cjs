...
(async()=>{
 ...
 dom.window.close();
 let chunkedBoot=await boot(snapshot,snapshot.asof,null,true);({dom,w,d}=chunkedBoot);assert.ok(chunkedBoot.urls.some(x=>/(^|\/)data\/health\.json/.test(x)));assert.ok(chunkedBoot.urls.every(x=>!x.includes('snapshots/unit/health.json')),'refresh health is read outside the immutable prior snapshot');d.querySelector('[data-stock]').click();await new Promise(r=>setImmediate(r));assert.ok(d.querySelector('#detail-chart svg'),'immutable chart chunk renders');dom.window.close();
