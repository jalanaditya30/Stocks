...
The expected session is the latest NSE calendar close at least three hours before scan time. A versioned calendar combines the maintained `pandas_market_calendars` NSE schedule with reviewed additions/removals in `config/exchange_calendar_overrides.json`. Unexpected closures or calendar errors require a reviewed override/version change; a failed benchmark freshness check stops publication. Historical benchmark continuity is checked against this independent calendar, never against the benchmark's own index.

Fetch two years of daily prices once per security per scan, with one targeted retry for missing symbols, stale latest dates or gaps inside the required recent-session window. Keep the more complete coherent revision; never splice downloads or synthesize OHLC. Normalize dates, remove invalid/duplicate OHLC bars, and discard all data after the expected session. The benchmark must be current; at least 85% of the configured universe must have current data. Each snapshot publishes per-date and per-symbol continuity diagnostics, with freshness, history and liquidity kept separate. A concentrated gap affecting at least 10% of the registry marks the snapshot degraded without weakening eligibility. A failed scan preserves the earlier successful snapshot and writes a separate health error.

Scanner-eligible stocks require 130 complete recent benchmark sessions, at least 55 positive-volume days in the last 60 sessions, and median 60-session close × volume of at least ₹2 crore/day. A separate context layer retains descriptive technical and volume analysis for registered stocks with at least 65 comparable sessions even when continuity, freshness or liquidity fails; those rows are labelled `Context only`, cannot become candidates, and do not enter theme statistics, rankings, detection records or backtests. Corporate-action-adjusted prices are used for returns and chart structure; raw closes are used for turnover. Close × volume is a cash-turnover approximation, not official exchange traded value.
...
Every stock with sufficient usable context history receives four visible, past-only context checks:
...
The separately versioned `review-priority-v1-shadow` ranking is displayed for comparison but does not replace that default order. It separates descriptive strength from fresh-entry priority, restricts priority to confirmed eligible candidates, gives N/A rather than zero for missing inputs, and never promotes extra names to fill a top ten. Dual-negative review blocks fresh-entry priority. Exact transforms, bounded component budgets, peer fallback and validation rules are frozen in [RANKING_SPEC.md](RANKING_SPEC.md). Theme state is deliberately excluded from the score. Rank changes compare only distinct sessions with the same score version; first-detection scores are preserved in the ledger.

Momentum stages are descriptive: Confirmed move, Trend continuation, Extended/event, Trend intact, Pullback/trend intact, Momentum weakening or Mixed. `Sector-supported setup`, `Hold / monitor`, `Sector review`, `Exit review` and `Watch` are research-posture labels, not broker instructions. `Exit review` requires both completed-close VStop and OBV MACD states to be bearish; a weak theme alone receives the distinct `Sector review` label. Any evaluated execution occurs at the following available open because the states are unknown before the signal close. Gaps, slippage and circuit limits can make an actual exit materially different.

The detail and group charts use the same completed-session adjusted OHLC series as the scanner, with traded volume below it on the same x-axis. Each refresh publishes the current calendar year beginning 1 January. VStop is calculated on the full available history before the chart is sliced, avoiding an artificial indicator restart on New Year's Day. Charts overlay VStop, the adjusted breakout reference and a first-detection marker when that date is inside the displayed period. Pointer or touch movement reveals the selected session's OHLC, close-to-close move and volume; this interaction is presentation only.

The All charts view can scan a browser-local saved watchlist, a selected curated theme or industry, or an uploaded custom symbol list. Custom text/CSV lists accept comma- or whitespace-separated NSE values such as `NSE:MOTILALOFS`; uploaded order is retained. BSE-prefixed tokens such as `BSE:SNL` are retained and explicitly reported as unsupported because the configured registry and market feed are NSE-based—they are not silently matched to a possibly different NSE symbol. A registered NSE symbol receives descriptive analysis when at least 65 comparable sessions exist; strict scanner failures are disclosed as context-only instead of suppressing the indicators. Symbols without enough usable history remain explicitly unavailable. Uploaded symbols remain in local browser storage and are included in workspace exports.
...
The Evidence view now leads with historical best-high / worst-low observations and prospective +5% / +10% reach rates. Historical portfolio exits remain separately available. Selection rules stay at `confirmed-v3-industry-rank`; this change does not retune rankings against the supplied charts.
...
The scanner follows TradingView's published `ta.vStop` logic and the open-source OBV MACD formulas, including source-value initialization before ATR exists, Wilder/SMA-seeded smoothing, and the OBV script's chart-bar denominator in its adaptive channel. Exact displayed values can still differ because the app uses Yahoo's completed daily adjusted history, while a TradingView screenshot may use a different feed, adjustment history or an unfinished intraday bar. Directional state is the intended comparison; numerical equality with a screenshot is not claimed.
...
The separate user-specified exit overlay sells at the next available open after both completed-close VStop and OBV MACD states are bearish, with 0.25% assumed cost on each side. It retains the same finite-capital entry portfolio and reports delayed missing opens. It was specified after the entry-model holdout had already been viewed, so it is a descriptive comparison rather than a fresh untouched validation. Its historical indicators use the full retained seven-year input; the live scanner downloads two years, so path-dependent states can differ at the margin.
