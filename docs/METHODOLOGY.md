...
The All charts view can scan a browser-local saved watchlist, a selected curated theme or industry, or an uploaded custom symbol list. Custom text/CSV lists accept comma- or whitespace-separated NSE values such as `NSE:MOTILALOFS`; uploaded order is retained. BSE-prefixed tokens such as `BSE:SNL` are retained and explicitly reported as unsupported because the configured registry and market feed are NSE-based—they are not silently matched to a possibly different NSE symbol. A symbol receives the same published analysis only when it is in the current eligible snapshot. Missing, thin, stale or unregistered symbols are disclosed rather than silently analysed under weaker rules. Uploaded symbols remain in local browser storage and are included in workspace exports.
